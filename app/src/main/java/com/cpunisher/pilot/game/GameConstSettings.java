package com.cpunisher.pilot.game;

public class GameConstSettings {

    public static final int START_HEART = 3;
    public static final int START_POWER = 1;
    public static final int MAX_POWER = 3;
    public static final int MAX_HEART = 5;

    public static final int BULLET_SPEED = 20;
    public static final int ENEMY_SPEED = 3;
    public static final int BACKGROUND_MOVE_SPEED = 3;

    public static final int EACH_SCORE = 10;
    public static final int SCORE_EACH_LEVEL = 200;

    // 1 tick = 20 ms
    public static final int ONE_TICK = 20;
    public static final int GOD_TICKS = 100;

    public static final int BATTERY_GOD_MODE = 200;
    public static final int GOD_MODE_FLASH = 15;
    public static final int PLAYER_SHOOT = 20;
    public static final int ENEMY_SHOOT = PLAYER_SHOOT * 3;
    public static final int GENERATE_ENEMY = 75;
    public static final int GENERATE_ITEM = 500;
}
